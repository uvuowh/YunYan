...
async function handleSubmit(): Promise<void> {
  ...
  try {
    ...
    if (success) {
      appStore.showSuccess('Welcome back!', 'You have been successfully logged in')
      
      // Redirect to intended page or dashboard
      const redirectTo = (route.query.redirect as string) || '/dashboard'
      await router.push(redirectTo)
    } else {
      appStore.showError('Login failed', 'Invalid email or password')
    }
  } catch (error) {
    console.error('Login error:', error)
    appStore.showError('Login failed', 'An unexpected error occurred')
  }
}
