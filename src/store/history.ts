...
export class RemoveCardCommand implements Command {
  ...
  execute(): void {
    ...
    // 删除相关连接
    this.canvasStore.connections.splice(
      0,
      this.canvasStore.connections.length,
...
export const useHistoryStore = defineStore('history', () => {
  ...
  const getUndoDescription = () =>
    undoStack.length > 0 ? undoStack[undoStack.length - 1].description : ''
  const getRedoDescription = () =>
    redoStack.length > 0 ? redoStack[redoStack.length - 1].description : ''
